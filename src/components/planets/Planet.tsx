...
const PLANET_CONFIGS: Record<PlanetVariant, PlanetConfig> = {
  ice: {
    radius: 1.45,
    atmosphereColor: '#b8d4ec',
    atmosphereLayers: [
      { scale: 1.05, intensity: 0.10, power: 2.8, softness: 0.9 },
      { scale: 1.12, intensity: 0.20, power: 1.6, softness: 0.75 },
      { scale: 1.24, intensity: 0.10, power: 1.1, softness: 0.55 },
    ],
    lightColor: '#88b8e8',
    rings: [
      { inner: 1.7, outer: 2.2, color: '#a8c8dc', opacity: 0.26, tilt: 0.42, gapCenter: 0.55, gapWidth: 0.04 },
      { inner: 2.25, outer: 2.75, color: '#8aa8c4', opacity: 0.12, tilt: 0.42 },
    ],
    moons: [
  ...
  gas: {
    radius: 1.55,
    atmosphereColor: '#f0c4b4',
    atmosphereLayers: [
      { scale: 1.04, intensity: 0.10, power: 2.6, softness: 0.88 },
      { scale: 1.11, intensity: 0.22, power: 1.5, softness: 0.72 },
      { scale: 1.22, intensity: 0.12, power: 1.0, softness: 0.5 },
    ],
    lightColor: '#e8a878',
    rings: [
      { inner: 2.05, outer: 2.45, color: '#d8b898', opacity: 0.22, tilt: 0.22, gapCenter: 0.5, gapWidth: 0.05 },
      { inner: 2.5, outer: 3.0, color: '#b89878', opacity: 0.10, tilt: 0.22 },
    ],
  ...
  terran: {
    ...
    atmosphereLayers: [
      { scale: 1.05, intensity: 0.10, power: 2.6, softness: 0.92 },
      { scale: 1.14, intensity: 0.20, power: 1.45, softness: 0.78 },
      { scale: 1.28, intensity: 0.12, power: 1.0, softness: 0.58 },
    ],
    lightColor: '#9898d8',
    rings: [
      { inner: 1.8, outer: 2.25, color: '#b0a8d0', opacity: 0.24, tilt: 0.58, gapCenter: 0.48, gapWidth: 0.035 },
      { inner: 2.3, outer: 2.85, color: '#9088b8', opacity: 0.11, tilt: 0.58 },
    ],
    moons: [
...
function AtmosphereShell({
  ...
  useFrame(() => {
    ...
    const softenEdge = hovered && mode === 'mini';
    const targetIntensity = layer.intensity * intensity;
    const targetPower = softenEdge ? layer.power * 0.62 : layer.power;
    const targetSoftness = softenEdge
      ? Math.min(layer.softness + 0.22, 1)
      : layer.softness;
    ...
    matRef.current.uniforms.uPower.value = THREE.MathUtils.lerp(
      matRef.current.uniforms.uPower.value,
      targetPower,
      0.06
    );
    matRef.current.uniforms.uSoftness.value = THREE.MathUtils.lerp(
      matRef.current.uniforms.uSoftness.value,
      targetSoftness,
      0.06
    );
  });
...
export function Planet({
  ...
  const hoverShaderTarget = mode === 'mini' ? 0.25 : 1;
  ...
  useFrame((state, delta) => {
    if (groupRef.current) {
      ...
      if (mode === 'mini') {
        ...
        const bob = hovered ? Math.sin(state.clock.elapsedTime * 1.6) * 0.035 : 0;
        groupRef.current.position.y = position[1] + bob;
        groupRef.current.position.x = position[0];
      } else if (hovered && mode === 'default') {
    ...
    >
      ...
      {mode !== 'background' && (
        <pointLight
          ...
          intensity={
            (hovered && mode !== 'mini' ? 1.4 : 0.7) * modeSettings.lightIntensity
          }
